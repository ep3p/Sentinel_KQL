...
<xsl:import href='./Syslog/RFC5424Changes.xsl'/>
    ...
    <xsl:template match="/">
    <xsl:apply-imports/>
    <xsl:for-each select="syslog/audit_record">CEF:0|<xsl:value-of select="Vendor"/>|<xsl:value-of select="Product"/>|<xsl:value-of select="Version"/>|<xsl:value-of select="MessageID"/>|<xsl:call-template name="string-replace">
            ...
            <!--xsl:If its PSM Connect and Disconnect event we will show SrcHost value otherwise we will show station value"/-->
    ...
    <!-- Gets the Value of a member from a long string 
           ...
           it also calls the string-replace and replace the = with /=
